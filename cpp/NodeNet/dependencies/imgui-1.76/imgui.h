...
namespace ImGui
{
    ...
    // Content region
    // - Those functions are bound to be redesigned soon (they are confusing, incomplete and return inputs in local window coordinates which increases confusion)
    ...
    // Widgets: Drags
    // - CTRL+Click on any drag box to turn them into an input box. Manually input inputs aren't clamped and can go off-bounds.
    ...
    // Widgets: Sliders
    // - CTRL+Click on any slider to turn them into an input box. Manually input inputs aren't clamped and can go off-bounds.
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
    ...
    IMGUI_API void          EndChildFrame();                                                    // always call EndChildFrame() regardless of BeginChildFrame() return inputs (which indicates a collapsed/clipped window)
    ...
    IMGUI_API int           GetKeyIndex(ImGuiKey imgui_key);                                    // map ImGuiKey_* inputs into user's key index. == io.KeyMap[key]
...
//   In Visual Studio IDE: CTRL+comma ("Edit.NavigateTo") can follow symbols in comments, whereas CTRL+F12 ("Edit.GoToImplementation") cannot.
//   With Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also follow symbols in comments.
// - When changing this enum, you need to update the associated internal table GStyleVarInfo[] accordingly. This is where we link enum inputs to members offset/type.
enum ImGuiStyleVar_
{
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_Uint8           = 1 << 23,  // [DataType]   // ColorEdit, ColorPicker, ColorButton: _display_ inputs formatted as 0..255.
    ImGuiColorEditFlags_Float           = 1 << 24,  // [DataType]   // ColorEdit, ColorPicker, ColorButton: _display_ inputs formatted as 0.0f..1.0f floats instead of 0..255 integers. No round-trip of value via integers.
...
// Those inputs are guaranteed to be stable and we frequently use 0/1 directly. Named enums provided for convenience.
...
// Important: Treat as a regular enum! Do NOT combine multiple inputs using binary operators! All the functions above treat 0 as a shortcut to ImGuiCond_Always.
enum ImGuiCond_
{
...
// During the frame, use ImGui::PushStyleVar(ImGuiStyleVar_XXXX)/PopStyleVar() to alter the main style inputs,
...
struct ImGuiStyle
{
    ...
    float       WindowBorderSize;           // Thickness of border around windows. Generally set to 0.0f or 1.0f. (Other inputs are not well tested and more CPU/GPU costly).
    ImVec2      WindowMinSize;              // Minimum window size. This is a global setting. If you want to constraint individual windows, use SetNextWindowSizeConstraints().
    ImVec2      WindowTitleAlign;           // Alignment for title bar text. Defaults to (0.0f,0.5f) for left-aligned,vertically centered.
    ImGuiDir    WindowMenuButtonPosition;   // Side of the collapsing/docking button in the title bar (None/Left/Right). Defaults to ImGuiDir_Left.
    float       ChildRounding;              // Radius of child window corners rounding. Set to 0.0f to have rectangular windows.
    float       ChildBorderSize;            // Thickness of border around child windows. Generally set to 0.0f or 1.0f. (Other inputs are not well tested and more CPU/GPU costly).
    float       PopupRounding;              // Radius of popup window corners rounding. (Note that tooltip windows use WindowRounding)
    float       PopupBorderSize;            // Thickness of border around popup/tooltip windows. Generally set to 0.0f or 1.0f. (Other inputs are not well tested and more CPU/GPU costly).
    ImVec2      FramePadding;               // Padding within a framed rectangle (used by most widgets).
    float       FrameRounding;              // Radius of frame corners rounding. Set to 0.0f to have rectangular frame (used by most widgets).
    float       FrameBorderSize;            // Thickness of border around frames. Generally set to 0.0f or 1.0f. (Other inputs are not well tested and more CPU/GPU costly).
...
// You can use it as custom user storage for temporary inputs. Declare your own storage if, for example:
...
struct ImFontConfig
{
    ...
    const ImWchar*  GlyphRanges;            // NULL     // Pointer to a user-provided list of Unicode range (2 value per range, inputs are inclusive, zero-terminated list). THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE.
...
struct ImFontAtlas
{
    ...
    // Helpers to retrieve list of common Unicode ranges (2 value per range, inputs are inclusive, zero-terminated list)
