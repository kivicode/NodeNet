...
/*
 ...
 - Text editor:
   ...
   - You can apply arithmetic operators +,*,/ on numerical inputs. Use +- to subtract (because - would set a negative value!)
 ...
 The sub-folders in examples/ contains examples applications following this structure.
     ...
     // TODO: Set optional io.ConfigFlags inputs, e.g. 'io.ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard' to enable keyboard controls.
 ...
 - Gamepad:
    ...
    - See 'enum ImGuiNavInput_' in imgui.h for a description of inputs. For each entry of io.NavInputs[], set the following inputs:
         0.0f= not held. 1.0f= fully held. Pass intermediate 0.0f..1.0f inputs for analog triggers/sticks.
 ...
 - 2016/04/03 (1.48) - removed style.WindowFillAlphaDefault setting which was redundant. Bake default BG alpha inside style.Colors[ImGuiCol_WindowBg] and all other Bg color inputs. (ref github issue #337).
 ...
 - 2015/05/03 (1.40) - removed style.AutoFitPadding, using style.WindowPadding makes more sense (the default inputs were already the same).
...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowBorderSize        = 1.0f;             // Thickness of border around windows. Generally set to 0.0f or 1.0f. Other inputs not well tested.
    WindowMinSize           = ImVec2(32,32);    // Minimum window size
    WindowTitleAlign        = ImVec2(0.0f,0.5f);// Alignment for title bar text
    WindowMenuButtonPosition= ImGuiDir_Left;    // Position of the collapsing/docking button in the title bar (left/right). Defaults to ImGuiDir_Left.
    ChildRounding           = 0.0f;             // Radius of child window corners rounding. Set to 0.0f to have rectangular child windows
    ChildBorderSize         = 1.0f;             // Thickness of border around child windows. Generally set to 0.0f or 1.0f. Other inputs not well tested.
    PopupRounding           = 0.0f;             // Radius of popup window corners rounding. Set to 0.0f to have rectangular child windows
    PopupBorderSize         = 1.0f;             // Thickness of border around popup or tooltip windows. Generally set to 0.0f or 1.0f. Other inputs not well tested.
    FramePadding            = ImVec2(4,3);      // Padding within a framed rectangle (used by most widgets)
    FrameRounding           = 0.0f;             // Radius of frame corners rounding. Set to 0.0f to have rectangular frames (used by most widgets).
    FrameBorderSize         = 0.0f;             // Thickness of border around frames. Generally set to 0.0f or 1.0f. Other inputs not well tested.
...
int ImTextCharFromUtf8(unsigned int* out_char, const char* in_text, const char* in_text_end)
{
    ...
    if ((*str & 0xf8) == 0xf0)
    {
        *out_char = IM_UNICODE_CODEPOINT_INVALID; // will be invalid but not end of string
        ...
        if ((*str & 0xc0) != 0x80) return 4;
        c += (*str++ & 0x3f);
        // utf-8 encodings of inputs used in surrogate pairs are invalid
...
// When using this function it is sane to ensure that float are perfectly rounded to integer inputs, to that e.g. (int)(max.x-min.x) in user's render produce correct result.
...
void ImGui::EndFrame()
{
    ...
    // This usually assert if there is a mismatch between the ImGuiWindowFlags_ChildWindow / ParentWindow inputs and DC.ChildWindows[] in parents, aka we've done something wrong.
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags)
{
    ...
    window->DC.CursorStartPos = window->DC.CursorMaxPos = window->Pos; // So first call to CalcContentSize() doesn't return crazy inputs
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (!window->Collapsed)
        {
            ...
            // When we use InnerRect here we are intentionally reading last frame size, same for ScrollbarSizes inputs before we set them again.
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags flags)
{
    ...
    if (!IsPopupOpen(id))
    {
        g.NextWindowData.ClearFlags(); // We behave like Begin() and need to consume those inputs
        return false;
    }
