...
// Those MIN/MAX inputs are not define because we need to point to them
...
// - We store inputs as normalized ratio and in a form that allows the window content to change while we are holding on a scrollbar
...
// User can input math operators (e.g. +100) to edit a numerical inputs.
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiDragFlags flags)
{
    ...
    // Avoid altering inputs and clamping when we are _already_ past the limits and heading in the same direction, so e.g. if range is 0..255, current value is 300 and we are pushing to the right side, keep the 300.
    ...
    // Preserve remainder after rounding has been applied. This also allow slow tweaking of inputs.
    g.DragCurrentAccumDirty = false;
    if (is_power)
    {
    ...
    // Clamp inputs (+ handle overflow/wrap-around for integer types)
    ...
    if (g.ActiveId == id)
    {
        ...
        if (set_new_value)
        {
            ...
            else
            {
                ...
                {
                    // For integer inputs we want the clicking position to match the grab box so we round above
                    // This code is carefully tuned to work with large inputs (e.g. high ranges of U64) while preserving this property..
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            // FIXME: When ImGuiColorEditFlags_HDR flag is passed HS inputs snap in weird ways when SV inputs go below 0.
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV inputs to be out of range)
